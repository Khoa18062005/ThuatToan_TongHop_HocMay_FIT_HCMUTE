...
def custom_knn(X_train, y_train, test_point, k, metric='euclidean', task='classification'):
    distances = []
...
if __name__ == '__main__':
    ...
    print("=> THỬ NGHIỆM DỰ ĐOÁN 1 MẪU:")
    ...
    new_flower_data = [6.5, 3.0, 5.2, 2.0]
    new_point = pd.Series(new_flower_data, index=features_cols)
    new_pred_class, nearest_neighbors = custom_knn(
        X_train, y_train, new_point,
        k=k_value, metric='euclidean', task='classification'
    )
    print("Thông số bông hoa mới thu thập được:")
    print(new_point.to_string())
    print(f"-> MÔ HÌNH DỰ ĐOÁN ĐÂY LÀ HOA: {new_pred_class.upper()}")

    # ĐÁNH GIÁ MÔ HÌNH
    print("=" * 45)
    print("ĐÁNH GIÁ ACCURACY (BÀI TOÁN PHÂN LOẠI)")

    # Dự đoán trên toàn bộ tập Train (dùng list comprehension trực tiếp thay vì tạo hàm riêng)
